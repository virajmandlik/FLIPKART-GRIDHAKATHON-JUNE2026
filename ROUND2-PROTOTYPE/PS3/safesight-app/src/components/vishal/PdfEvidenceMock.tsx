...
export default function PdfEvidenceMock({ compact }: Props) {
  ...
  return (
    <m.div
      ...
      className={`gov-card overflow-hidden ${compact ? "" : ""}`}
    >
      <div className="border-b border-white/[0.06] bg-ink-900/50 px-4 py-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded border border-saffron/25 bg-saffron/10">
              <FileText className="h-4 w-4 text-saffron" />
            </span>
            <div>
              <div className="gov-label">Court exhibit</div>
              <h4 className="font-semibold text-white">BTP Evidence Certificate</h4>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {!compact && (
              <button
                ...
                className="rounded border border-white/[0.08] px-3 py-1.5 text-xs font-medium text-slate-400 hover:text-white"
              >
            ...
            <button
              type="button"
              onClick={generatePdf}
              className="inline-flex items-center gap-1.5 rounded bg-saffron px-3 py-1.5 text-xs font-bold text-ink-950 hover:brightness-105"
            >
      ...
      <div className="border-b border-dashed border-white/[0.08] px-4 py-3">
        <div className="flex flex-wrap items-center gap-3 text-[11px]">
          <span className="rounded border border-amber-brand/30 px-2 py-0.5 font-semibold text-saffron">BSA S.63(4)</span>
      ...
      {previewOpen && (
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4">
          <div className="rounded border border-white/[0.08] bg-ink-950 p-4">
            <div className="mb-2 flex items-center gap-2 gov-label">
              <Shield className="h-3 w-3" /> Document preview
            </div>
            <pre className="max-h-56 overflow-auto font-mono text-[10px] leading-relaxed text-slate-400">
              {previewLines.join("\n")}
            </pre>
          </div>
        </m.div>
