    ...
    <div className="space-y-5">
      ...
      {isLowLight && (
        ...
        <motion.div
          ...
          className="rounded-xl bg-teal/10 p-4 ring-1 ring-teal/30"
      ...
      {isPagdi && (
        ...
        <motion.div
          ...
          className="rounded-xl bg-ok/10 p-4 ring-1 ring-ok/40"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-ok">AUTO-CLEARED · Pagdi exempt</span>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <motion.div
          ...
          className={`relative overflow-hidden rounded-2xl ring-1 ${isPagdi ? "ring-ok/30" : "ring-danger/30"}`}
        ...
        >
          ...
          <div className="absolute bottom-3 right-3">
            ...
            <Pill tone={isPagdi ? "ok" : "danger"}>

              {isPagdi ? "0 violations" : `${violations.length} violations`}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl glass p-5">
            ...
            <div className="text-xs font-bold uppercase tracking-wider text-slate-500">Vehicle & plate</div>

            <div className="mt-3 flex flex-wrap items-center gap-3">

              <span className="text-lg font-bold capitalize text-white">{data.vehicle.type}</span>

              {!isPagdi && <Pill tone="cyan">{(data.vehicle.confidence * 100).toFixed(0)}% conf</Pill>}

            </div>

            <div className="mt-4 rounded-xl bg-saffron/10 px-4 py-3 ring-2 ring-saffron/40">

              <div className="text-[10px] font-bold uppercase tracking-wider text-saffron">ANPR · PaddleOCR</div>

              <div className="mt-1 font-mono text-2xl font-extrabold tracking-widest text-white">{plate}</div>

              <div className="mt-1 text-xs text-slate-400">

                BLR_CAM_023 · Silk Board → Marathahalli
          ...
          <div className={`rounded-2xl glass p-5 ring-1 ${isPagdi ? "ring-ok/30" : "ring-ok/20"}`}>

            <div className="flex items-center gap-2">

              <Bot className="h-5 w-5 text-teal" />

              <span className="text-sm font-bold text-white">Agentic validation</span>
      ...
      {!isPagdi && (
        ...
        <div className="rounded-xl bg-risk-high/10 p-4 ring-1 ring-risk-high/30">

          <div className="flex items-center gap-2 text-xs font-bold text-risk-high">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {Object.entries(data.rule_engine_results)
              ...
              .map(([k]) => (
                ...
                <span key={k} className="rounded-md bg-danger/15 px-2 py-1 text-[11px] font-semibold text-danger">

                  {VIOLATION_LABELS[k] ?? k.replace(/_/g, " ")}
...
function MiniStat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  ...
  return (
    ...
    <div className={`rounded-lg p-3 ring-1 ${warn ? "bg-danger/10 ring-danger/30" : "bg-white/[0.03] ring-white/[0.06]"}`}>

      <div className="text-[10px] uppercase text-slate-500">{label}</div>

      <div className={`text-sm font-bold ${warn ? "text-danger" : "text-white"}`}>{value}</div>
